# JavaInterview
  ...
  * [Spring篇](#spring篇)
    ...
    * [10、简述一下Spring Bean的生命周期](#10简述一下spring-bean的生命周期)
    ...
    * [14、Spring如何解决循环依赖？](#14spring如何解决循环依赖)
      * [第一种：构造器循环依赖](#第一种构造器循环依赖)
      * [第二种：setter方式单例](#第二种setter方式单例)
    * [15、BeanFactory 和ApplicationContext的区别](#15beanfactory-和applicationcontext的区别)
    * [16、Spring 自动装配Bean原理.](#16spring-自动装配bean原理)
    * [17、Spring中 @Autowired原理](#17spring中-autowired原理)
    * [18、Spring事务的传播机制](#18spring事务的传播机制)
    * [19、Spring事务隔离级别](#19spring事务隔离级别)
  * [MyBatis篇](#mybatis篇)
    ...
    * [7、 如何执行批量插入？](#7-如何执行批量插入)
        * [方案1：【个人未验证】](#方案1个人未验证)
        * [方式2：mapper接口传入List,sql中使用for 循环插入](#方式2mapper接口传入listsql中使用for-循环插入)
    ...
    * [12、Mybatis如何返回插入主键？](#12mybatis如何返回插入主键)
      * [情况1：主键为整型自增类型](#情况1主键为整型自增类型)
      * [情况2：使用的是UUID或者自定义的一套varchar类型的唯一ID](#情况2使用的是uuid或者自定义的一套varchar类型的唯一id)
    * [13、Mybatis每一个mapper.xml都会对应一个mapper或者dao接口，讲讲它的原理](#13mybatis每一个mapperxml都会对应一个mapper或者dao接口讲讲它的原理)
      * [SqlsessionFactory](#sqlsessionfactory)
      * [Sqlsession](#sqlsession)
      * [MapperRegistry](#mapperregistry)
      * [MapperProxyFactory](#mapperproxyfactory)
      * [MapperProxy](#mapperproxy)
      * [MapperProxy.invoke()](#mapperproxyinvoke)
      * [MapperMethod.execute() 执行CRUD](#mappermethodexecute-执行crud)
  ...
  * [MySQL篇](#mysql篇)
    ...
    * [17、为什么mysql的索引使用B+树，而MongoDB中使用B树](#17为什么mysql的索引使用b树而mongodb中使用b树)
      * [B树](#b树)
      * [B+树](#b树-1)
  * [Redis篇](#redis篇)
    * [0、Redis中的数据类型](#0redis中的数据类型)
      * [5种数据类型](#5种数据类型)
      * [常用命令](#常用命令)
          * [单机登录](#单机登录)
          * [集群登录](#集群登录)
...
②各种开源框架如hibernate，struts等框架使用反射实现的。 
...
代理模式是常用的Java设计模式，他的特征是代理类与委托类有同样的接口，代理类主要负责为委托类预处理消息、过滤消息、把消息转发给委托类，以及事后处理消息等。代理类与委托类之间通常会存在关联关系，一个代理类的对象与一个委托类的对象关联，代理类的对象本身并不真正实现服务，而是通过调用委托类的对象的相关方法，来提供特定的服务。 【类比客户委托中介找房的过程便可以很好的理解】
...
将程序中的交叉业务逻辑（比如权限安全控制，日志，事务、防重复提交、动态切换数据源、性能分析等），封装成一个切面，然后注入到目标对象（具体业务逻辑）中去。
...
**五、注解注入** 常用的注入方式。从spring2.5开始，增加了注解注入，常用的@Autowired和@Resource两个注解可以实现Bean的注解



### 4、讲一下什么是Spring

Spring是一个轻量级的IOC和AOP容器框架。是为Java应用程序提供基础性服务的一套框架，目的是用于简化企业应用程序的开发，它使得开发者只需要关心业务需求。常见的配置方式有三种：基于XML的配置、基于注解的配置、基于JavaConfig的配置。
...
**4、处理器Handler(需要工程师开发)--即Controller**
...
**6、视图View(需要工程师开发jsp...)**
View是一个接口，实现类支持不同的View类型（jsp、freemarker、pdf...），现在业界通用的方案一般都是前后端进行分离，用JSON进行交互。
...
@Controller:作用于类上面，表明该类为控制器类

@RestController：作用于类上，表明该类为控制器类。与@Controller的区别在于，使用该注解的类中的所有方法都默认使用了@ResponseBody
...
**切面(Aspect)**：抽取具有某一特定公共功能【例如日志记录、事务、数据源切换、防重复提交等】做成切面类。切面是切点和通知的组合称谓，就是变相给组合起了个名字。 使用@Aspect 注解修饰。

**切入点(Pointcut)**：切点就是我们我们配置的满足我们条件的目标方法。比如我们规定：名字前面是select开头的才执行我们自定义的通知方法。

​            那么这些select开头的方法就是切点。

**连接点(JoinPoint)**：连接点可以说是切点的全集。切点是连接点的子集。也可以理解为，连接点是我们没有定义那个select开头规则时，满足条件的全部的方法。

**通知(Advice)**： 就是我们编写的希望Aop时执行的那个方法。我们通过Aop希望我们编写的方法在目标方法执行前执行，或者执行后执行。所以又可以细分为：
...
public class CheckRepeatedSubmitAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (signature.getDeclaringType().isAnnotationPresent(CheckRepeatedSubmit.class)
                || signature.getMethod().isAnnotationPresent(CheckRepeatedSubmit.class)) {
            ...
            if (redisTools.hasKey(sha256)) {
                BaseApiVo<Object, String> baseApiVo = new BaseApiVo<>();
                baseApiVo.setCode(ReturnCode.REPEAT_SUBMIT.getCode());
                baseApiVo.setMessage(ReturnCode.REPEAT_SUBMIT.getDescription());
                return baseApiVo;
            } else {
...
PS:
1、@Aspect 修饰CheckRepeatedSubmitAspect代表为一个切面
2、@Pointcut 定义了一系列的切入点，而切入点中定义的规则作用的对象就是目标对象
3、所有的 @Pointcut在一起形成了连接点ProceedingJoinPoint
4、@Around(...)环绕通知作用于定义的所有@Pointcut上
5、执行完整个 @Around(...)方法就完成了织入的过程   
...
（3）request：为每一个请求创建一个实例，在请求完成以后，bean会失效并被垃圾回收器回收。
...
AOP 思想的底层实现技术，Spring 中采用 JDK Proxy 和 CgLib 类库。
...
SpringMVC 提供了 DispatcherServlet 来对请求进行分发。【委派模式不是23种设计模式中的，但是在Spring中使用比较广泛】

**⑤简单工厂模式**

BeanFactory 用来创建对象的实例，贯穿于 BeanFactory / ApplicationContext接口的核心理念。
...
Spring的事件驱动模型使用的就是观察者模式，最常用的地方就是各个listener(如ApplicationListener)的实现。

更多Spring原理的问题参考：https://www.cnblogs.com/benbencyb/p/11755393.html
...
```java
<bean id="a" class="cn.xutingyin.entity.A">
    <constructor-arg name="b" ref="b"/>
</bean>
<bean id="b" class="cn.xutingyin.entity.B">
    <constructor-arg name="c" ref="c"/>
</bean>
<bean id="c" class="cn.xutingyin.entity.C">
...
![](https://img-blog.csdn.net/20170301154450018)

从上图中我们可以看到，Spring容器是先将Bean对象实例化之后再设置对象的属性的。

```xml
  //默认scope 为singleton 单例 
	<bean id="a" class="cn.xutingyin.entity.A">
...
```java
cn.xutingyin.entity.A@5a8e6209
```

为什么这里就不会存在循环依赖问题呢？

我们结合上面那张图看，Spring先是用构造器实例化Bean对象 ，此时Spring会将这个实例化结束的对象放到一个Map中，并且Spring提供了获取这个未设置属性的实例化对象引用的方法(提前暴露)。 

结合我们的实例来看，当Spring实例化了A、B、C后，紧接着会去设置对象的属性，此时A依赖B，就会去Map中取出存在里面的单例B对象，以此类推，不会出现循环的问题。

我们查看DefaultSingletonBeanRegistry.java的源码也可以得到验证：
...
如果我们将scope 改为非singleton,如prototype，会出现什么情况呢？

```xml
  //默认scope 为singleton 单例 
	<bean id="a" class="cn.xutingyin.entity.A" scope="prototype">
        <property name="b" ref="b"/>
    </bean>
    <bean id="b" class="cn.xutingyin.entity.B" scope="prototype">
        <property name="c" ref="c"/>
    </bean>
    <bean id="c" class="cn.xutingyin.entity.C" scope="prototype">
        <property name="a" ref="a"/>
    </bean>
```
...
### 16、Spring 自动装配Bean原理.

### 17、Spring中 @Autowired原理

核心实现类：AutowiredAnnotationBeanPostProcessor
...
public enum Propagation {
    ...
    //  表示如果当前正有一个事务在进行中，则该方法应当运行在一个嵌套式事务中。被嵌套的事务可以独立于封装事务进行提交或回滚。
    // 如果封装事务不存在，行为就像PROPAGATION_REQUIRES一样。
	......
}
```
...
由枚举类Isolation 进行管理，其实Spring的事务隔离级别是一一对应数据库中的事务隔离级别。

```java
public enum Isolation {
	
    // 默认值-读已提交：表示使用底层数据库的默认隔离级别。对大部分数据库而言，通常这值就是：READ_COMMITTED
	DEFAULT(TransactionDefinition.ISOLATION_DEFAULT),			
	
 	// 读未提交：该隔离级别表示一个事务可以读取另一个事务修改但还没有提交的数据。该级别不能防止脏读和不可重复读，因此很少使用该隔离级别.
	READ_UNCOMMITTED(TransactionDefinition.ISOLATION_READ_UNCOMMITTED),
 	
    // 读已提交：该隔离级别表示一个事务只能读取另一个事务已经提交的数据。该级别可以防止脏读，这也是大多数情况下的推荐值。
	READ_COMMITTED(TransactionDefinition.ISOLATION_READ_COMMITTED),
	
	// 可重复读：该隔离级别表示一个事务在整个过程中可以多次重复执行某个查询，并且每次返回的记录都相同。
...
Spring的事务隔离级别、传播特性、异常回滚类等都在@Transactional注解上设置，如果是使用的基于xml的配置，则在配置文件中进行配置。
...
**string:**常用于缓存、限流、计数器、分布式锁、分布式session
...
```shell
redis-cli -h 127.0.0.1 -p 7001 -c   # -c 表示以集群方式登录
```
...
4. 中文使用楷体、英文、数字使用Times New Roman 字体
